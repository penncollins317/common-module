...
import top.echovoid.security.oauth2.authentication.DeviceClientAuthenticationConverter;
import top.echovoid.security.oauth2.authentication.DeviceClientAuthenticationProvider;
import top.echovoid.security.oauth2.federation.OidcUserInfoExt;
    ...
    public static KeyPair getKeyPair() {
        try {
            if (new File(PRIVATE_KEY_FILE).exists() && new File(PUBLIC_KEY_FILE).exists()) {
                ...
                log.info("public key md5:{}", HashUtils.md5(publicKeyBytes));
                log.info("private key md5:{}", HashUtils.md5(privateKeyBytes));
